...
func getAttachedTagsOnObjects(ctx *CheckContext, vctx *validationContext, referencesToCheck []mo.Reference) ([]vapitags.AttachedTags, error) {
	...
	var filteredAttachedTags []vapitags.AttachedTags
	if vctx.regionAffinity != nil {
		for _, attachedTag := range attachedTags {
			fmt.Printf("type: %s\n", attachedTag.ObjectID.Reference().Type)
		}
	}

	if vctx.zoneAffinity != nil {
		for _, attachedTag := range attachedTags {
			fmt.Printf("type: %s\n", attachedTag.ObjectID.Reference().Type)
			filteredAttachedTags = append(filteredAttachedTags, attachedTag)
		}
	}
