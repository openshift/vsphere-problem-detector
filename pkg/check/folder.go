...
func CheckFolderPermissions(ctx *CheckContext) *CheckError {
	...
	errCheck := listDirectory(ctx, ds, "/", false)
	if err != nil {
		return errCheck
	}

	// OCP needs permissions to list "/kubelet", tolerate if it does not exist.
	errCheck = listDirectory(ctx, ds, "/kubevols", true)
	if err != nil {
		return errCheck
	}
	return nil
...
func listDirectory(ctx *CheckContext, ds *object.Datastore, path string, tolerateNotFound bool) *CheckError {
